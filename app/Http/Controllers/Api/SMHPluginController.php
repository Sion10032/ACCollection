...
class SMHPluginController extends Controller
{
    ...
    public function search($name) {
        ...
        $result_count = QueryList::html($html)->rules(array('res' => array('div.result-count>strong', 'html')))->query()->getData()->all()[1]['res'];

        $books = [];

        for($p = 1; count($books) < $result_count; $p++){
            $url = self::$site . '/s/' . urlencode($name) . '_p' . $p . '.html';
            if($p != 1) $html = self::getHtml($url);

            $booksLi = QueryList::html($html)->rules(array('html' => array('li.cf', 'html')))->query()->getData();
            ...
            foreach($booksLi as $book){
                $tmp = QueryList::html($book["html"])->rules($rules_book)->query()->getData()->all()[0];
    ...
    public function book($bid){
        ...
        $menucate = QueryList::html($html)->rules(array(
        ...
        $dir = [];
        $dir['bid'] = $bid;
        $dir['name'] = QueryList::html($html)->rules(array(array("div>h1", "text")))->query()->getData()->all()[0][0];
        $dir['dir'] = [];

        foreach($menucate as $menu){
            $menulist = QueryList::html($menu['menulist'])
                ->rules(array(
            ...
            array_push($dir['dir'], array('cateName' => $menu['category'], 'menuList' => $menulist));
        }
    ...
    public function chapter($bid, $cid){
        ...
        $js = str_replace('window["\x65\x76\x61\x6c"](', 'var res = ', $data_json[0]['js']);
        ...
        return str_replace("\\/", "/", json_encode($pics));
    }
